		...
		try {
          ...
          const checkLimit = (sender) => {
           ...
           return false
     	   ...
     	   /*[--Salida automática del formulario GC si el miembro está por debajo del límite  --]*/
 	       if (isGroup) {
				try {
					const getmemex = groupMembers.length	
				    if (getmemex <= memberlimit) {
					reply(`Lo sentimos, los miembros del grupo no han calificado. Al menos los miembros del grupo son ${memberlimit}`)
					setTimeout( () => {
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    try { 
                        ...
                        setTimeout( () => {
								...
								setTimeout( () => {
								client.updatePresence(from, Presence.composing)
								reply("*「 ANTI BADWORD 」*\nEres un dikick para decir palabras groseras !")
							}, 0)
                        } catch { client.sendMessage(from, `Afortunadamente el no es admin, si admin tiene veredicto, saquelo!`, text , {quoted : mek}) }
                } else {
                    return reply( "Por favor, cuide el discurso de Min 😇")
                }
            }
        }
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					if (isGroupAdmins) return reply('Dado que eres un administrador de grupo, los bots no te patearán')
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`Link Group Terdeteksi maaf ${sender.split("@")[0]} Te echarán del grupo a 5 en segundos`)
						setTimeout( () => {
			...
			switch(command) { 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					setTimeout( () => {
					client.sendMessage(from, adivinar, text, {quoted: mek})
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_Quedan 10 Segundos..._', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_Quedan 20 Segundos..._', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_Quedan 30 Segundos..._', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
